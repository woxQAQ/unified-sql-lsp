...
mod tests {
    ...
    impl<N> Lowering<N> for MockLowering
    where
        N: CstNode,
    {
        fn lower_query(&self, _ctx: &mut LoweringContext, node: &N) -> LoweringResult<Query> {
            if node.kind() == "select_statement" {
                Ok(Query::new(Dialect::MySQL))
        ...
        fn lower_expr(&self, ctx: &mut LoweringContext, node: &N) -> LoweringResult<Expr> {
            if node.kind() == "column_ref" {
                Ok(Expr::Column(unified_sql_lsp_ir::ColumnRef::new("id")))
            } else if node.kind() == "unsupported" {
                ctx.add_error(LoweringError::UnsupportedSyntax {
                    dialect: "Mock".to_string(),
                    feature: "unsupported expression".to_string(),
                    suggestion: "Use a different expression".to_string(),
                });
    ...
    #[test]
    fn test_trait_lower_query_success() {
    ...
    #[test]
    fn test_trait_lower_query_failure() {
        let lowering = MockLowering;
        let mut ctx = LoweringContext::new(Dialect::MySQL);
    ...
    #[test]
    fn test_trait_lower_expr_success() {
        let lowering = MockLowering;
        let mut ctx = LoweringContext::new(Dialect::MySQL);
        let node = MockCstNode::new("column_ref");

        let result = lowering.lower_expr(&mut ctx, &node);
        assert!(result.is_ok());

        if let Ok(Expr::Column(col)) = result {
    ...
    #[test]
    fn test_trait_lower_expr_placeholder() {
        let lowering = MockLowering;
        let mut ctx = LoweringContext::new(Dialect::MySQL);
        let node = MockCstNode::new("unknown");
