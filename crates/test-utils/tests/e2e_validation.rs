...
fn get_test_base_path() -> String {
    // Determine the base path for test fixtures
    // Tests run from crates/test-utils, fixtures are at ../../tests/e2e/fixtures
    let manifest_dir = std::env::var("CARGO_MANIFEST_DIR").unwrap_or_else(|_| ".".to_string());
    format!("{}/../../tests/e2e/fixtures", manifest_dir)
}
...
#[test]
fn test_basic_select_file_structure() {
    let base_path = get_test_base_path();
...
#[test]
fn test_join_foreign_key_context() {
    let base_path = get_test_base_path();
...
#[test]
fn test_functions_dialect_coverage() {
    let base_path = get_test_base_path();
...
fn test_edge_cases_boundary_conditions() {
    ...
    let syntax_error_test = cases
        .iter()
        .find(|c| c.description.contains("syntax error") || c.description.contains("misspelled"));
...
fn test_options_validation() {
    ...
    assert!(
        with_options.is_some(),
        "01_basic_select.txt should have tests with options"
    );
...
fn test_dialect_distribution() {
    ...
    for spec in TEST_FILES {
        ...
        let cases = unified_sql_lsp_test_utils::parse_test_file(Path::new(&path))
            .expect(&format!("Failed to parse {}", spec.name));
